...
import org.apache.samza.job.dm.{Enforcer, EnforcerFactory}
import org.apache.samza.util.hadoop.HttpFileSystem
import org.apache.samza.util.Logging

import scala.collection.JavaConverters._

class YarnEnforcerFactory extends StreamJobFactory with Logging with EnforcerFactory {
  def getJob(config: Config) = {
    ...
    new YarnEnforcer(config, hConfig)
  }

  /**
    * submit job to the cluster and return the enforcer URL
    */
  override def getEnforcer(config: Config): Enforcer = {
    getJob(config)
  }
}
