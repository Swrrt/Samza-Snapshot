...
public class MetricsLagRetriever {
    ...
    private void updateProcessed(JSONObject json){
        ...
        if (taskMetrics != null) {
            ...
            long currentTime = json.getJSONObject("header").getLong("time");
            long currentProcessed = taskMetrics.getLong("messages-total-processed"); //taskMetrics.getLong("messages-actually-processed") + getLastProcessed(taskNameToPartition(taskName)) - getBegin(taskNameToPartition(taskName));
